...
try {
    $perlCheck = perl -e "print 'OK'" 2>$null
    if ($perlCheck -eq "OK") {
        Write-Host "✓ Perl détecté et fonctionnel" -ForegroundColor Green
    } else {
        throw "Perl test failed"
    }
} catch {
...
foreach ($file in $files.GetEnumerator()) {
    ...
    $syntaxResult = perl -I"$sourceDir" -c "$($file.Value)" 2>&1
...
foreach ($module in $requiredModules) {
    $moduleResult = perl -M"$module" -e "exit 0" 2>$null
...
foreach ($script in $testScripts) {
    ...
    if (Test-Path $scriptPath) {
        ...
        $syntaxResult = perl -I"$sourceDir" -c "$scriptPath" 2>&1
        
        if ($LASTEXITCODE -eq 0) {
            Write-Host " OK" -ForegroundColor Green
        } else {
            Write-Host " ERREUR" -ForegroundColor Red
            Write-Host "Erreur: $syntaxResult" -ForegroundColor Red
        }
    } else {
